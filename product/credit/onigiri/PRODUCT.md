...
| [Application Approval](capabilities/application-approval/CAPABILITY.md) | Product / Engineering | Draft | Routes `pending_approval` applications to the correct approver role based on risk level. Provides role-scoped approver worklist and role-gated application detail view (7 visibility tiers). Captures Approve / Reject / Request Document Upload decisions with immutable audit trail. Strict role matching — no upward delegation. |
| [Restructure Pre-Approval](capabilities/restructure-pre-approval/CAPABILITY.md) | Product / Engineering | Draft | Sales team builds a restructuring plan for an existing loan contract and optionally submits it to CA for pre-approval before the customer fills in an application. A CA-approved plan lowers the application's RAE risk level to 10–20, routing to SALE_BRANCH/SALE_AREA instead of CA. Includes plan builder, CA worklist modal (separate from application list), detail view, and Approve/Change/Reject decision flow with sales re-confirmation on Change. |
