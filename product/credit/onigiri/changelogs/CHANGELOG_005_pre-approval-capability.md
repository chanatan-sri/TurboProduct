...
| EasyPass bypasses approval — does not auto-convert | EasyPass means the approval step is bypassed (no Approval Request submitted). The CO still explicitly converts the pre-approval to Draft. "Auto-convert on plan selection" was incorrect and has been corrected across all specs. |
| EasyPass is a campaign type designation, not a stored flag | EasyPass is indicated by the campaign type returned by Campaign Eligibility Pre-Build — not stored as a boolean `easypass_flag` field on the pre-approval or application record. Routing at `pending_approval` and change detection conditions are evaluated against the campaign type of the selected campaign. `easypass_flag` removed from all record field definitions. |
...
| CO Worklist removed as entry point — replaced by Pre-Approval Status Visibility in BOS | Pre-approval has no application number. CO Worklist surfaces items by application reference — there is nothing to surface before a Draft is created. Instead, BOS Customer Detail displays pre-approval status directly on the customer record (badge on Customer List; status section on Customer Detail). CO converts from Customer Detail without needing a separate worklist. |
| Pre-Approval Status Visibility added as new feature | Requires BOS to surface pre-approval state on Customer List (badge indicator) and Customer Detail (status section with state, plan, expiry, and Convert to Draft action). Closes the gap left by removing CO Worklist — CO is passively informed when approval status changes without navigating to every customer. |
...
- `capabilities/pre-approval/features/FEATURE_pre-approval-status-visibility.md` — created (new: replaces CO Worklist entry point)
